...
TweetManager.prototype = {
  ...
  */
  ...
  showTweetsNotifications: function(tweetsToNotify, forceOnPage) {
    ...
    //console.warn('style: ' + notificationStyle);
    c//onsole.warn('force? ' + forceOnPage);
    if (notificationStyle != 'desktop') {
        //console.warn('forcing notification style to desktop');
    ...
    if(!forceOnPage && notificationStyle == 'desktop') {
        ...
        for(i = 0; i < tweetsToNotify.length; ++i) {
            var opt = {
                type: "basic",
                title: tweetsToNotify[i].user.name,
                message: tweetsToNotify[i].text,
                iconUrl: tweetsToNotify[i].user.profile_image_url
            };
            chrome.notifications.create('', opt, function(id) {
                setTimeout(function() { chrome.notifications.clear(id, function() {}) }, fadeTimeout);
            });
        }
    } else {
